...
class Scraper extends Model {
	...
	private function _parseEventDonostiaEUS($event, $language) {
		...
		if (strpos($event['price'], 'Gratis') !== false) {
$event['price'] = '0 €';
		}
	...
	private function _addCategoriesFromUrlAndTitle($urlCat, $title) {
		...
		if (Functions::strpos_array($haystack, $theatreSlugs) !== false) {
			$categories[] = 'Teatro y Danza';
		}

		// Cinema
		$cinemaSlugs = array('cine', 'película', 'film');
		if (Functions::strpos_array($haystack, $cinemaSlugs) !== false) {
			$categories[] = 'Cine';
		}

		// Music / Concerts
		$musicSlugs = array('concierto', 'kontzertu', 'música', 'musika');
		if (Functions::strpos_array($haystack, $musicSlugs) !== false) {
			$categories[] = 'Música';
		}

		// Sports
		$sportSlugs = array('deporte', 'kirol', 'carrera', 'karrera', 'maratón', 'maratoia');
		if (Functions::strpos_array($haystack, $sportSlugs) !== false) {
			$categories[] = 'Deportes';
		}

		// Expositions
		$expositionSlugs = array('exposicion', 'exposición', 'erakusketa');
		if (Functions::strpos_array($haystack, $expositionSlugs) !== false) {
			$categories[] = 'Exposiciones';
		}

		// Parties
		$partySlugs = array('fiesta', 'feria', 'festa', 'azoka', 'mercadillo');
		if (Functions::strpos_array($haystack, $partySlugs) !== false) {
			$categories[] = 'Fiestas y Ferias';
		}

		// Gastronomy
		$gastronomySlugs = array('gastro', 'pinchos', 'pintxoak');
		if (Functions::strpos_array($haystack, $gastronomySlugs) !== false) {
			$categories[] = 'Gastronomía';
		}

		// Children
		$childrenSlugs = array('infantil', 'haur', 'famili');
		if (Functions::strpos_array($haystack, $childrenSlugs) !== false) {
			$categories[] = 'Actividades Infantiles';
		}

		// Museums
		$museumSlugs = array('museo');
		if (Functions::strpos_array($haystack, $museumSlugs) !== false) {
			$categories[] = 'Museos';
		}

		// Conferences
		$conferenceSlugs = array('conferencia', 'hitzaldi', 'konferentzia');
		if (Functions::strpos_array($haystack, $conferenceSlugs) !== false) {
			$categories[] = 'Conferencias';
		}

		// Literature
		$literatureSlugs = array('literatura', 'lector', 'irakur');
		if (Functions::strpos_array($haystack, $literatureSlugs) !== false) {
